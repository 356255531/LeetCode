...
public:
    ...
    public:
        ...
        std::vector<int>::const_iterator _m_stone_iterator;
        long long _m_previous_step_length;

        Node(const std::vector<int>::const_iterator& which_stone, 
            const long long& how_long_step_length)
        {   
            _m_stone_iterator = which_stone;
            _m_previous_step_length = how_long_step_length;
        }
        bool operator < (const Node& node_a) const
        {
            if (*(node_a._m_stone_iterator) == *_m_stone_iterator)
                return (node_a._m_previous_step_length < _m_previous_step_length);

            return (*(node_a._m_stone_iterator) < *_m_stone_iterator);
        }
        ~Node(){};
    ...
    bool canCross(const std::vector<int>& stones) {
        ...
        std::deque<Node> node_query;
        std::set<Node> node_visited;
        bool if_crossed = false;
        long long temp_step_length;

        Node current_node(stones.cbegin() + 1, 1);
        node_query.push_back(current_node);
        node_visited.insert(current_node);

        while(node_query.size() != 0)
        {
            current_node = node_query.front();
            node_query.pop_front();

            if (*(current_node._m_stone_iterator) == stones.back())
            {
                if_crossed = true;
                break;
            }

            for (std::vector<int>::const_iterator i = current_node._m_stone_iterator + 1; 
                    i != stones.cend(); ++i)
            {
                temp_step_length = *i - *(current_node._m_stone_iterator);
                if (temp_step_length > current_node._m_previous_step_length + 1) 
                    break;
                else if (temp_step_length < current_node._m_previous_step_length - 1)
                    continue;

                if (node_visited.find(Node(i, temp_step_length)) 
                    == node_visited.cend())
                {
                    node_query.push_back(Node(i, temp_step_length));
                    node_visited.insert(Node(i, temp_step_length));
